...
//thunks
export const getTasks = (tId: string): ThunkType => {
...
export const deleteTask = (tId: string, taskId: string): ThunkType => {
  return async (dispatch) => {
...
export const createTask = (title: string, taskId: string): ThunkType => {
  return async (dispatch) => {
...
export const updateTaskTitle = (todoId: string, taskId: string, title: string): ThunkType => {
  return async (dispatch, getState) => {
    ...
    const model: UpdateTaskModelType = {...task, status: task.status}
    TodolistsAPI.updateTitleTask(todoId, taskId, model).then((data) => {
      dispatch(changeTasksTitleAC(todoId, taskId, title));
    });
  }
}
export const changeTaskStatus = (todoId: string, taskId: string, status: TaskStatus): ThunkType => {
  return async (dispatch, getState) => {
    const state = getState();
    const task = state.tasks[todoId].find( t => t.id === taskId)
    if(!task) {
